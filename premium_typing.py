...
# ------------- Utility: OpenAI filler generator ---------------
def generate_filler(goal_text, min_words=3, max_words=10):
    """
    Returns a plausible, on-topic filler phrase using OpenAI or dummy fallback.
    """
    try:
        prompt = (
            ...
            if random.random() < edit_chance:
                ...
                if idx > 0 and goal_text[idx-1].isalpha() and not ch.isspace():
                    ...
                    while idx < length and goal_text[idx].isalpha():
                        idx += 1
                    # Now ch will be next word boundary
                    if idx >= length:
                        break
                    ch = goal_text[idx]
                ...
                filler = generate_filler(goal_text, FILLER_MIN_WORDS, FILLER_MAX_WORDS)
                for fch in filler:
